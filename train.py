...
def get_arguments():
    ...
    parser.add_argument('--data_dir', type=str, default=DATA_DIRECTORY,
                        help='The directory containing the data corpus for training.')
    parser.add_argument('--test_dir', type=str, default=TEST_DIRECTORY,
                        help='The directory containing the data corpus for testing.')
...
def main():
    ...
    with tf.name_scope('create_inputs'):
        ...
        reader = FileReader(args.data_dir, args.test_dir, coord)
    ...
    # regularization
    #regularizers = tf.nn.l2_loss(net.vars['conv0']['w']) + tf.nn.l2_loss(net.vars['conv00']['w']) + \
    #               tf.nn.l2_loss(net.vars['fc1']['w']) + tf.nn.l2_loss(net.vars['out']['w'])
    ...
    optim = optimizer.minimize(loss=loss, var_list=trainable)
    #gradient = optimizer.compute_gradients(loss, var_list=trainable)
    # optim = tf.contrib.layers.optimize_loss(
    ...
    writer_test = tf.summary.FileWriter(
        get_default_logdir(LOGDIR_ROOT, 'test'))
    ...
    # will use 3 for train and 1 for validation queues
    reader.start_threads(sess, n_threads=16)
    ...
    try:
        ...
        for step in range(saved_global_step + 1, args.num_steps):
            ...
            if args.store_metadata and step % 50 == 0:
                ...
                loss_value_train, _ = sess.run([loss, optim],
                                               {reader.queue_switch(): 0, net.train_phase(): True})
            #                writer_train.add_summary(summary, step)

            if step % 100 == 0:
                loss_value_test = sess.run(loss,
                                           {reader.queue_switch(): 1, net.train_phase(): False})

                summary_train = sess.run(summaries,
                                         {reader.queue_switch(): 0, net.train_phase(): False})
                writer_test.add_summary(summary_train, step)
        ...
        input_node = sess.graph.get_tensor_by_name(
            "create_model/radnet_1/input_node:0")
        ...
        converted_graph_def = graph_util.convert_variables_to_constants(
            sess, graph_def, ["create_model/radnet_1/out/output_node"])
